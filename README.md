...
2) Each training sample consist of a certain number of USD, Pound and Euro prices as input values and WIG trend for the next day. The trend is not a price itself but a label representing how much the value has changed (big decrease, small decrease, small increase, big increase). Thanks to this approach, the model can return decent results without the need to average the input data, and can also be used without recurrent layers.
#
...
2) Model returns a value of sinus function after receiving its argument.

Both examples are provided with a python script that plots the model results in a certain numberical space.
